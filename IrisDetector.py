...
def get_darkest_area(image):
    ...
    # only check the darkness of a block for every Nth x and y pixel (sparse sampling)
    imageSkipSize = 10
    searchArea = 20  # the size of the block to search
    # skip every Nth x and y pixel in the local search area (sparse sampling)
    internalSkipSize = 5
    ...
    for y in range(ignoreBounds, gray.shape[0] - ignoreBounds, imageSkipSize):
        for x in range(ignoreBounds, gray.shape[1] - ignoreBounds, imageSkipSize):
            ...
            if current_sum < min_sum and num_pixels > 0:
                min_sum = current_sum
                darkest_point = (x + searchArea // 2, y +
                                 searchArea // 2)  # Center of the block
...
def optimize_contours_by_angle(contours, image):
    ...
    for i in range(0, len(all_contours), 1):
        ...
        prev_point = all_contours[i - spacing] if i - \
            spacing >= 0 else all_contours[-spacing]
        next_point = all_contours[i + spacing] if i + \
            spacing < len(all_contours) else all_contours[spacing]
...
def fit_and_draw_ellipses(image, optimized_contours, color):
    if len(optimized_contours) >= 5:
        ...
        contour = np.array(optimized_contours,
...
def check_contour_pixels(contour, image_shape, debug_mode_on):
    ...
    cv2.ellipse(ellipse_mask_thick, ellipse, (255),
                10)  # capture more for absolute
    ...
    absolute_pixel_total_thick = np.sum(
        overlap_thick > 0)  # compute with thicker border
    absolute_pixel_total_thin = np.sum(
        overlap_thin > 0)  # compute with thicker border

    # Compute the ratio of pixels under the ellipse to the total pixels on the contour border
    total_border_pixels = np.sum(contour_mask > 0)

    ratio_under_ellipse = absolute_pixel_total_thin / \
        total_border_pixels if total_border_pixels > 0 else 0
...
def check_ellipse_goodness(binary_image, contour, debug_mode_on):
    ...
    ellipse_goodness[2] = min(ellipse[1][1]/ellipse[1]
                              [0], ellipse[1][0]/ellipse[1][1])
...
def fit_ellipse_with_given_center(points, center):
    if len(points) < 5:
        raise ValueError(
            "At least 5 contour points are required to fit an ellipse.")
...
class Smaller_Circle_Info():
    def __init__(self):
        self.ellipse = None


biggerCircleInfo = Bigger_Circle_Info()
smallerCircleInfo = Smaller_Circle_Info()
...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, render_cv_window, iris):
    ...
    image_array = [thresholded_image_relaxed, thresholded_image_medium,
                   thresholded_image_strict]  # holds images
    ...
    for i in range(1, 4):
        ...
        if iris:
            dilated_image = cv2.dilate(
                image_array[i-1], kernel, iterations=7)  # Large
            dilated_image = cv2.erode(image_array[i-1], kernel, iterations=1)
        else:
            dilated_image = cv2.dilate(
                image_array[i-1], kernel, iterations=2)  # medium
        # Find contours
        ...
        reduced_contours = filter_contours_by_area_and_return_largest(
            contours, 500, 3)
        if iris:
            cont.append(reduced_contours)
        if len(reduced_contours) > 0 and len(reduced_contours[0]) > 5:
            current_goodness = check_ellipse_goodness(
                dilated_image, reduced_contours[0], debug_mode_on)
            ...
            total_pixels = check_contour_pixels(
                reduced_contours[0], dilated_image.shape, debug_mode_on)
            ...
            final_goodness = current_goodness[0] * \
                total_pixels[0]*total_pixels[0]*total_pixels[1]

            # show intermediary images with text output
            # if debug_mode_on:
            if True:
                cv2.putText(gray_copies[i-1], "%filled:     " + str(current_goodness[0])[
                            :5] + " (percentage of filled contour pixels inside ellipse)", (10, 30), font, .55, (255, 255, 255), 1)  # %filled
                cv2.putText(gray_copies[i-1], "abs. pix:   " + str(total_pixels[0]) +
                            " (total pixels under fit ellipse)", (10, 50), font, .55, (255, 255, 255), 1)  # abs pix
                cv2.putText(gray_copies[i-1], "pix ratio:  " + str(total_pixels[1]) +
                            " (total pix under fit ellipse / contour border pix)", (10, 70), font, .55, (255, 255, 255), 1)  # abs pix
                cv2.putText(gray_copies[i-1], "final:     " + str(final_goodness) +
                            " (filled*ratio)", (10, 90), font, .55, (255, 255, 255), 1)  # skewedness
    ...
    p = [process_frames.center_x, process_frames.center_y]
    if final_contours and not isinstance(final_contours[0], list) and len(final_contours[0] > 5):
        ...
        if not iris:
            # smaller circle
            smallerCircleInfo.ellipse = ellipse
            cv2.ellipse(test_frame, ellipse, (55, 255, 0), 2)
            (x, y), (process_frames.MA,
                     process_frames.ma), process_frames.angle = ellipse
        ...
        if iris:
            ...
            std_dev = [np.std(distances[i: i + size])
            ...
            std_dev0 = [np.std(dist0[i: i + size])
                        for i in range(0, len(dist0), step)]
            std_dev1 = [np.std(dist1[i: i + size])
                        for i in range(0, len(dist1), step)]
            std_dev2 = [np.std(dist2[i: i + size])
                        for i in range(0, len(dist2), step)]
    ...
    if iris and detected_iris:
        ...
        indices = [index for index, value in enumerate(
            std_dev) if 0.8 < value < 10]

        indices0 = [index for index, value in enumerate(
            std_dev0) if 0.8 < value < 10]
        if indices0 == []:
        ...
        indices1 = [index for index, value in enumerate(
            std_dev1) if 0.8 < value < 10]
        if indices1 == []:
            sorted_lookup = sorted(enumerate(std_dev1), key=lambda i: i[1])
            if len(sorted_lookup) >= 2:
                indices1 = [sorted_lookup[0][0], sorted_lookup[1][0]]

        indices2 = [index for index, value in enumerate(
            std_dev2) if 0.8 < value < 10]
        if indices2 == []:
        ...
        min_std_con_points = contour_points[index_min *
                                            step:index_min*step+size, :]
        ...
        if result_array0 != [] or result_array1 != [] or result_array2 != []:
            all_contours_iris_dists = [
                result_array0, result_array1, result_array2]
            ...
            cv2.ellipse(test_frame, tuple(map(int, p)),
                        (int(axes[0]), int(axes[1])),
                        process_frames.angle,
                        0, 360,
                        (55, 255, 0),
                        3)

            biggerCircleInfo.center = p
            biggerCircleInfo.axes0 = int(axes[0])
            biggerCircleInfo.axes1 = int(axes[1])
            biggerCircleInfo.angle = process_frames.angle
            # else:
            # cv2.circle(test_frame, p, int(radius),  (0,255,0), 2)
            cv2.drawContours(
                gray_copies[a-1], min_std_con_points.reshape(-1, 1, 2), -1, (55, 255, 0), 3)
            cv2.drawContours(
                gray_copies[0], cont_point0_array, -1, (55, 255, 0), 3)
            cv2.drawContours(
                gray_copies[1], cont_point1_array, -1, (55, 255, 0), 3)
            cv2.drawContours(
                gray_copies[2], cont_point2_array, -1, (55, 255, 0), 3)
    ...
    if render_cv_window and iris:
        ratio = round(((axes[0])*(axes[1])) /
                      (process_frames.MA*process_frames.ma), 3)
        # cv2.putText(test_frame, "%IRIS / PUPIL (AREA): " + str(ratio) +
        #             "(percentage of Iris over Pupil)", (10, 30), font, 0.6, (0, 0, 0), 1)  # %filled
        ratioResult.result = ratio
        # cv2.imshow('best_thresholded_image_contours_on_frame', test_frame)
    # Create an empty image to draw contours
...
def save_processed_image(image, output_path=None):
    """
    Save the processed image to a file.
    If no output_path is provided, saves to the current directory with a timestamp.
    """
    if output_path is None:
        from datetime import datetime
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_path = f'processed_image_{timestamp}.png'
...
def process_frame(frame_input):
    debug_mode_on = False
    temp_center = (0, 0)

    # Check if input is a path or image
    ...
    while True:
        ...
        thresholded_image_strict = apply_binary_threshold(
            gray_frame, darkest_pixel_value, 5)  # lite
        thresholded_image_strict = mask_outside_square(
            thresholded_image_strict, darkest_point, 250)

        thresholded_image_medium = apply_binary_threshold(
            gray_frame, darkest_pixel_value, 15)  # medium
        thresholded_image_medium = mask_outside_square(
            thresholded_image_medium, darkest_point, 250)

        thresholded_image_relaxed = apply_binary_threshold(
            gray_frame, darkest_pixel_value, 25)  # heavy
        thresholded_image_relaxed = mask_outside_square(
            thresholded_image_relaxed, darkest_point, 250)

        # take the three images thresholded at different levels and process them
        pupil_rotated_rect, center_x, center_y, test_frame = process_frames(
            thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, True, False)

        radius = int(10 * (pupil_rotated_rect[1][0] // 2))

        thresholded_image_strict = apply_binary_threshold(
            gray_frame, darkest_pixel_value, 120)  # lite
        thresholded_image_strict = mask_outside_square(
            thresholded_image_strict, [center_x, center_y], radius)

        thresholded_image_medium = apply_binary_threshold(
            gray_frame, darkest_pixel_value, 140)  # medium
        thresholded_image_medium = mask_outside_square(
            thresholded_image_medium, [center_x, center_y], radius)

        thresholded_image_relaxed = apply_binary_threshold(
            gray_frame, darkest_pixel_value, 160)  # heavy
        thresholded_image_relaxed = mask_outside_square(
            thresholded_image_relaxed, [center_x, center_y], radius)

        pupil_rotated_rect, center_x, center_y, test_frame = process_frames(
            thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, test_frame, gray_frame, darkest_point, debug_mode_on, True, True)

        # Save the processed frame
        # success, path = save_processed_image(test_frame)
        ...
        #             break
        return test_frame, ratioResult.result
        break
    cv2.destroyAllWindows()
...
def process_video(video_path, input_method):
    ...
    # Output video filename, codec, frame rate, and frame size
    out = cv2.VideoWriter(
        'C:/Storage/Source Videos/output_video.mp4', fourcc, 30.0, (640, 480))
...
#         video_path = filedialog.askopenfilename(title="Select Video File", filetypes=[
#                                                 ("Video Files", "*.mp4;*.avi")])
#         if not video_path:
...
#     #     frame_path = filedialog.askopenfilename(title="Select Frame File", filetypes=[
#     #                                             ("Video Files", "*.jpeg;*.avi")])
#     #     if not frame_path:
#     #         print("No file selected. Exiting.")
#     #         return
...
#     frame_path = str(args.image_path)
#     # second parameter is 1 for video 2 for webcam
...
#     select_frame()
    ...
    # circle_data = {
    #     "smaller_circle": {
    #         "center": list(smallerCircleInfo.ellipse[0]),
    #         "axes": list(smallerCircleInfo.ellipse[1]),
    #         "angle": smallerCircleInfo.ellipse[2]
    #     },
    #     "bigger_circle": {
    #         "center": [biggerCircleInfo.center[0], biggerCircleInfo.center[1]],
    #         "axes": [biggerCircleInfo.axes0, biggerCircleInfo.axes1],
    #         "angle": biggerCircleInfo.angle
    #     }
    # }

    # with open('ellipse_params.json', 'w') as f:
    #     json.dump(circle_data, f, indent=4)

    # print("Smaller Circle")
    # print(smallerCircleInfo.ellipse)
#
    # print("Bigger Circle")
    # print(((biggerCircleInfo.center[0], biggerCircleInfo.center[1]), biggerCircleInfo.axes0,
    #    biggerCircleInfo.axes1, biggerCircleInfo.angle))
# _, result = process_frame('./1.jpeg')
# print(result)
